...
//////////////////////pie_ points   
    ...
    image(yum, c5X, c5Y, cWidth, cHeight);//pie 5
	if (p1x >= c5X-cWidth/2 && p1x <= c5X+cWidth/2 && p1Y-cHeight/2 && p1Y <= c5Y+cHeight/2) {
		score = score +1 ;//char hit coin 
		c5X = -1000; //make pie disapear 
		yumSound.play();
	} 
...
console.log('Pressed')
if (key === 'a') {
    p1x -= 50; // Move left faster
}
else if (key === 'd') {
    p1x += 50; // Move right faster
}
   else if (key ==='w'){
        console.log('jumping')
        jump = true; //jump 
